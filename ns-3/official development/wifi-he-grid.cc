...
NS_LOG_COMPONENT_DEFINE ("he-wifi-grid");
...
int main (int argc, char *argv[]) {
	...
	cmd.AddValue ("allStas", "Probar de 3 en 3 todas las STAs hasta 'numStas'", allStas);
        cmd.AddValue ("mcs", "Limitar pruebas a MCS específico (0-11)", mcs);
	cmd.AddValue ("gi", "Limitar pruebas a GI específico (800, 1600, 3200 ns)", gi);
        cmd.AddValue ("channelWidth", "Limitar pruebas a canal de ancho específico (20, 40, 80, 160 MHz)", channelWidth);
	cmd.AddValue ("useRts", "Activar/desactivar RTS/CTS", useRts);
	cmd.AddValue ("useExtendedBlockAck", "Activar/desactivar uso de BACK extendido", useExtendedBlockAck);
	...
	for (; stas <= numStas; stas += 3) {
		for (int mcs = minMcs; mcs <= maxMcs; mcs++) {
			...
			for (int channelWidth = minChannelWidth; channelWidth <= maxChannelWidth; ) { //MHz
				for (int gi = maxGi; gi >= minGi; ) { //Nanoseconds
					...
					std::cout << "MCS value" << "\t\t" << "Channel width" << "\t\t" << "GI" << "\t\t" << "Agg. Throughput" << '\n';
					std::cout << mcs << "\t\t\t" << channelWidth << " MHz\t\t\t" << gi << "\t\t\t" << aggThroughput << " Mbit/s" << std::endl;
					std::cout << '\n';
