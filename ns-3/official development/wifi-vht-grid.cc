...
NS_LOG_COMPONENT_DEFINE ("vht-wifi-grid");
...
int main (int argc, char *argv[]) {
	...
	cmd.AddValue ("allStas", "Probar de 3 en 3 todas las STAs hasta 'numStas'", allStas);
        cmd.AddValue ("mcs", "Limitar pruebas a MCS específico (0-9)", mcs);
	...
	if (channelWidth >= minChannelWidth && channelWidth <= maxChannelWidth) {
		if (channelWidth == 20 && mcs == 9) {
                        NS_LOG_ERROR ("MCS 9 no puede ser usado con canal de 20 MHz!");
                        exit (1);
                } else if (channelWidth != 20 && channelWidth != 40 && channelWidth != 80 && channelWidth != 160) {
