...
#define  _ENABLE_DEBUG_   1 // Valor original: 0
...
uint32_t numPackets = 10; // Valor original: 1000000
uint32_t voipPacketSize = 38; // bytes
uint32_t dataPacketSize = 1460; // bytes // Valor original: 200
double simulatorBeginRangeStart = 2; //Seconds
double simulatorBeginRangeEnd = 2; //Seconds // Valor original: 2
...
double simulatorDuration = 10; //Seconds // Valor original: 5
uint32_t  nVoipStas = 0;
uint32_t  nVideoStas = 0;
uint32_t  nDataStas = 0;
bool  downlink = true; // Valor original: false
  ...
  while ((m_receivedPacket = socket->Recv ()))
    {
        for (std::vector<Ptr<Socket>>::size_type rit = 0; rit < dataRecvSockPtr.size(); rit++) {
            NS_LOG_UNCOND("Valor de rit: " << rit);
            NS_LOG_UNCOND("Valor de dataRecvSockPtr.size(): " << dataRecvSockPtr.size());
            if(socket == dataRecvSockPtr[rit]) {
                memset(recvBuffer, '\0', sizeof(recvBuffer));
                recvBytes = m_receivedPacket->CopyData(recvBuffer, 16);
                //NS_LOG_UNCOND("Bytes recibidos: " << recvBytes);

                NS_ASSERT(recvBytes);
                bufferToData64(&startTime, recvBuffer, 0);  //Decode from 0th byte // Valor original: primer parámetro &startTime
...
static void GenerateDataTraffic (uint32_t pktSize, Time pktInterval, uint32_t staIndex)
{
  ...
  //NS_LOG_UNCOND("!!! Llamada a función GenerateDataTraffic !!!");
  ...
  if(rit < dataRecvSockPtr.size()) {
      if(dataPktStats[rit].currentPktCount < dataPktStats[rit].maxPktCount) {
           ...
           NS_LOG_UNCOND("Tamaño de paquete: " << pktSize);
           ...
           NS_LOG_UNCOND("Tamaño de buffer: " << buffer);
...
{
  double frec = 2.4;
  std::string phyMode ("11"); // Valor original: HeMcs0
  std::string numStas ("1");
  bool verbose = false;
  ...
  cmd.AddValue ("frec", "Frecuencia (2.4, 5)", frec);
  cmd.AddValue ("phyMode", "Wifi Phy mode", phyMode);
  cmd.AddValue ("numStas", "# STAS (1, 5, 10, 15, 20, 25, 30)", numStas);
  cmd.AddValue ("downlink", "DL=true, UL=false", downlink);
  ...
  Config::SetDefault ("ns3::WifiNetDevice::Mtu", UintegerValue (1500)); // Valor original: 800

  cmd.Parse (argc, argv);

  phyMode = "HeMcs" + phyMode;
  ...
  std::ifstream file("scenarios/scenario-" + numStas + "sta.txt");
  std::string str;
  ...
  while (std::getline(file, str))
  {
    ...
    // --- Equivalente a nodeC.Create (cantidad)
    ...
    if (staType == "AP")
    {
      ...
      wifiMac.SetType ("ns3::ApWifiMac",
                       "Ssid", SsidValue (ssid));
      // Configurar algoritmo de control de velocidad
  ...
  resultsLog.open("output-logs/resultsFile.txt", std::ios_base::app);
  ...
  // Tracing
  wifiPhy.EnablePcap ("output-pcaps/wifi-simple-infra-" + std::to_string(runNumber) + "-run", devices);
  ...
  std::ofstream throughput_voip("output-logs/throughputVoip.txt");
  std::ofstream throughput_fullBuffer("output-logs/throughputFullBuffer.txt");
  std::ofstream throughput_video("output-logs/throughputVideo.txt");
  std::ofstream throughputBarPlot("output-plots/throughputBarPlot");
  std::ofstream ruliBarPlot("output-plots/ruliBarPlot");

  std::ofstream xlocation_var("output-logs/xlocation.txt");
  std::ofstream ylocation_var("output-logs/ylocation.txt");

  std::ofstream table_create("output-logs/tableData.txt");
  std::ofstream table_create_combined("output-logs/tableDataCombined.txt");

  std::ofstream drop_voip;
  std::ofstream drop_fullBuffer;
  std::ofstream drop_video;
  drop_voip.open ("output-logs/dropVoip.txt");
  drop_fullBuffer.open ("output-logs/dropFullBuffer.txt");
  drop_video.open ("output-logs/dropVideo.txt");
  std::ofstream drop_voip_per;
  std::ofstream drop_fullBuffer_per;
  std::ofstream drop_video_per;
  drop_voip_per.open ("output-logs/dropVoipPer.txt");
  drop_fullBuffer_per.open ("output-logs/dropFullBufferPer.txt");
  drop_video_per.open ("output-logs/dropVideoPer.txt");
  ...
  lat_voip.open ("output-logs/LatVoipWorst.txt");
  lat_voip_97.open ("output-logs/LatVoipWorst97.txt");
  lat_voip_99.open ("output-logs/LatVoipWorst99.txt");
  lat_fullBuffer.open ("output-logs/LatFullBufferAvg.txt");
  lat_video.open ("output-logs/LatVideoWorst.txt");
  lat_video_97.open ("output-logs/LatVideoWorst97.txt");
  lat_video_99.open ("output-logs/LatVideoWorst99.txt");
  ...
  jit_voip.open ("output-logs/JitVoipWorst.txt");
  jit_voip_97.open ("output-logs/JitVoipWorst97.txt");
  jit_voip_99.open ("output-logs/JitVoipWorst99.txt");
  jit_fullBuffer.open ("output-logs/JitFullBufferAvg.txt");
  jit_video.open ("output-logs/JitVideoWorst.txt");
  jit_video_97.open ("output-logs/JitVideoWorst97.txt");
  jit_video_99.open ("output-logs/JitVideoWorst99.txt");
  ...
  histfile_latency.open ("output-logs/histfileLatencyVoip.txt");
  histfile_jitter.open ("output-logs/histfileJitterVoip.txt");
  ...
  histfile_latency.open ("output-logs/histfileLatencyFullBuffer.txt");
  histfile_jitter.open ("output-logs/histfileJitterFullBuffer.txt");
  for (std::vector<pktStats_t>::size_type rit = 0; rit < dataPktStats.size(); rit++) {
      NS_LOG_UNCOND("Valor de rit: " << rit);
      NS_LOG_UNCOND("Valor de dataPktStats.size(): " << dataPktStats.size());
  ...
  histfile_latency.open ("output-logs/histfileLatencyVideo.txt");
  histfile_jitter.open ("output-logs/histfileJitterVideo.txt");
  ...
  std::ifstream qDropLog("output-logs/queueDrops.txt");
  char inStr[255];
  while(qDropLog) {
  ...
  std::ofstream allDropVoip("output-logs/DropsAllVoip.txt");
  std::ofstream allDropFullBuffer("output-logs/DropsAllFullBuffer.txt");
  std::ofstream allDropVideo("output-logs/DropsAllVideo.txt");
  for(uint32_t staIndex = 0; staIndex < nVoipStas; staIndex ++ ) {
    NS_LOG_INFO("VoIP STAs drop total: " << pktStats[staIndex].dropTotal);
    NS_LOG_INFO("VoIP STAs drop before queue: " << pktStats[staIndex].dropBeforeQueue);
    NS_LOG_INFO("VoIP STAs drop after queue: " << pktStats[staIndex].dropAfterQueue);
  ...
  NS_LOG_INFO("-----");
  for(uint32_t staIndex = 0; staIndex < nDataStas; staIndex ++ ) {
    NS_LOG_INFO("Data STAs drop total: " << dataPktStats[staIndex].dropTotal);
    NS_LOG_INFO("Data STAs drop before queue: " << dataPktStats[staIndex].dropBeforeQueue);
    NS_LOG_INFO("Data STAs drop after queue: " << dataPktStats[staIndex].dropAfterQueue);
  ...
  NS_LOG_INFO("-----");
  for(uint32_t staIndex = 0; staIndex < nVideoStas; staIndex ++ ) {
    NS_LOG_INFO("Video STAs drop total: " << videoPktStats[staIndex].dropTotal);
    NS_LOG_INFO("Video STAs drop before queue: " << videoPktStats[staIndex].dropBeforeQueue);
    NS_LOG_INFO("Video STAs drop after queue: " << videoPktStats[staIndex].dropAfterQueue);
  ...
  if(downlink == true) {
    ...
    table_create << nVoipStas << "," << nVideoStas << "," << nDataStas<< "," << "Uplink" << "," << location << "," << "17dBm" << "," << highLoadingAxResourceUL / usedAxResourceUL * 100 << ","; // PONGO UL
  }
